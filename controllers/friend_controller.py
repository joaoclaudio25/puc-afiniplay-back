...
@friend_bp.get("/check-email")
@token_required
def check_friend_email(current_user):
    """Usado pelo frontend antes de enviar o formulário: diz se esse e-mail já
    pertence a uma conta AfiniPlay, para trocar o convite por uma adição direta."""
    email = (request.args.get("email") or "").strip()
    if not email:
        return jsonify({"error": "Informe o e-mail."}), 400

    user = FriendService.find_registered_user_by_email(email)
    return jsonify({
...
def create_friend(current_user):
    ...
    existing_user = FriendService.find_registered_user_by_email(email)

    # Convite (e-mail/WhatsApp) só faz sentido para quem ainda não tem conta — se o
    # e-mail já pertence a um usuário, o amigo é vinculado direto (FriendService.create
    # já cria com status "registered" e a amizade recíproca), sem exigir forma de convite.
    if not existing_user:
        if not invite_via_email and not invite_via_whatsapp:
            return jsonify({"error": "Escolha ao menos uma forma de convite (e-mail ou WhatsApp)."}), 400
